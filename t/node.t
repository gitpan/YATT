...
#XXX: ok($att->get_name eq 'foo', "attribute name");
#XXX: ok($att->get_value eq 'bar', "attribute name");
#XXX: ok($att->get_quote eq 0, "attribute quote code");
...
#XXX: is_deeply [$att->children], [qw(bar baz)], 'list attribute->children';
#XXX: is $att->children, 2, 'scalar attribute->children';
...
my $i = 0;
ok(stringify_node($elem[$i++]) eq q(href="foo"), q(href="foo"));
ok(stringify_node($elem[$i++]) eq q(name='bar'), q(name='bar'));
ok(stringify_node($elem[$i++]) eq q(id=baz), q(id=baz));
